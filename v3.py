...
# Задайте веса для частей адреса
weights = {
...
# Вычисляем взвешенную схожесть и ранжируем адреса
weighted_similarities = []
for query, matched in zip(query_addresses, matched_addresses):
    query_parts = query.split()
    matched_parts = matched.split()
    weighted_similarity = 0
    for part, weight in weights.items():
        if part in query_parts and part in matched_parts:
            similarity = np.dot(X_query[0, vectorizer.vocabulary_[part]], X_train[0, vectorizer.vocabulary_[part]])
            weighted_similarity += similarity * weight
    weighted_similarities.append(weighted_similarity)

# Добавляем столбец с взвешенной схожестью в DataFrame запросов
query_df['weighted_similarity'] = weighted_similarities

# Сортируем DataFrame по убыванию взвешенной схожести
